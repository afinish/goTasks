...
func UpdateContact(updateContact *Contact) {
	contactID := strconv.FormatInt(updateContact.ID, 10)
	...
	_, e := config.DB.Exec(update, contact.FirstName, contact.LastName, contact.Phone, contact.Email, contact.Position, contactID)
	if e != nil {
		panic(err)
	}
}
